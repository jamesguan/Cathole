...
5. The Cathole workspace build using Noise NK over TCP (`transport.type = "noise"`):
   one control channel plus a pool of per-visitor Noise data channels (Rathole-style),
   with UDP still multiplexed on the control channel. Set `type = "quic"` in the
   Cathole configs to measure QUIC/UDP instead.
